...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    api.get('/api/v1/admin/users?limit=20').then(r => setUsers(r.data)).catch(() => {})
  }, [])
                ...
                <tbody>
                  {users.map(u => (
                    <tr key={u.id} className="border-b border-[#1e2a3a]/50 hover:bg-[#1e2a3a]/30">
                      ...
                      <td className="py-3 px-4">
                        <span className={`text-xs px-2 py-0.5 rounded-full ${u.status === 'active' ? 'bg-green-500/20 text-green-400' : u.status === 'suspended' ? 'bg-red-500/20 text-red-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                          {u.status}
                        </span>
                      </td>
